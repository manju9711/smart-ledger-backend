...
$admin_id    = intval($data['admin_id']   ?? 0);   // ✅ NEW
...
/* ── Validation ── */
if (!$company_id || !$admin_id || !preg_match('/^[0-9]{10}$/', $phone)) {
    echo json_encode(["status" => false, "message" => "Invalid customer data"]);
    exit;
}

/* ── Check if customer already exists (same phone + admin) ── */
$check = $conn->query("
    SELECT id FROM customers
    WHERE phone = '$phone' AND admin_id = '$admin_id' AND is_deleted = 0
    LIMIT 1
");
